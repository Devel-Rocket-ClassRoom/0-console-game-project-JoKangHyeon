...
namespace Framework.Tetris
{
    internal class TetrisObject : GameObject
    {
        ...
        public TetrisObject(Scene scene, int screenPosX, int screenPosY, BoardObject board) : base(scene)
        {
            ...
            for (int i = 0; i < drawTarget.GetLength(0); i++)
            {
                for (int j = 0; j < drawTarget.GetLength(1); j++)
                {
                    if (drawTarget[i, j])
                    {
                        ...
                        if (realY - TetrisGame.k_YBuffer < 0)
                            continue;
        ...
        public List<ConsoleColor> _minoColors = new() { ConsoleColor.Blue, ConsoleColor.Yellow, ConsoleColor.Red, ConsoleColor.Green, ConsoleColor.DarkBlue, ConsoleColor.DarkYellow, ConsoleColor.Magenta };

        public Dictionary<ShapeType, List<bool[,]>> _minoShapes = new()
        {
            {
                ShapeType.I, new(){
